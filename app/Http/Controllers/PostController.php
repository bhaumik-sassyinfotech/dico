    ...
    public function update($id, Request $request) {
        ...
        try {
            if (Auth::user()) {
                ...
                if ($res) {
                    /*                     * ************ web notification ******************* */
                    $commentUser = Comment::where('post_id', $id)->where('user_id', '!=', Auth::user()->id)->groupBy('user_id')->get();
                    if ($commentUser != null) {
                        foreach ($commentUser as $postMyUser) {
                            event(new \App\Events\PostUpdate(Auth::user(), $postMyUser));
                        }
                    }
                    ...
                    if ($res) {
                        ...
                        if (count($postDetail) > 0) {
                            $groups_id = GroupUser::select('user_id')
                                    ...
                                    ->whereIn('group_id', explode(',', $postDetail->group_id))
                                    ->where('is_admin', 1)
                                    ->groupBy('user_id')
                                    ->get();
                            if (count($groups_id) > 0) {
                                $companyUserDetail = User::where('is_suspended', '!=', 1)->get();
                                if (count($companyUserDetail) > 0) {
                                    foreach ($companyUserDetail as $companyAdmin) {
                                        event(new \App\Events\InappropriatePostCompanyAdmin(Auth::user(), $companyAdmin, $post_id));
                                    }
                                }
                            }
                        }
                        ...
                        if (count($commentUser) > 0) {
                            foreach ($commentUser as $postMyUser) {
                                event(new \App\Events\InappropriatePost(Auth::user(), $postMyUser));
                            }
                        }
                        /*                         * ************ end web notification ******************* */
                        DB::commit();
